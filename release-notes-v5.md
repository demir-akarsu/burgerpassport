...
- Fixed country flags showing blank (🏳️) for users whose device language
  isn't English — flags are now derived from the location's ISO country
  code directly instead of matching an English-only country name lookup,
  which never matched for a country name returned in another language.
  A one-time data fix was also run to correct existing affected burgers
- Fixed the profile picture cropper: it previously couldn't zoom out past
  filling the entire screen (far more restrictive than the actual crop
  area needed), and once that was corrected, panning/zooming had no
  bounds at all — the photo could be dragged or zoomed until the crop
  circle showed empty space instead of the photo. Both are now properly
  constrained to the photo's actual edges
- Fixed the profile picture picker intermittently showing a blank sheet
  instead of the crop screen, most often on the first attempt after
  opening it — a timing race between the photo picker's own dismissal
  and the crop screen's presentation
- Scoring sliders now snap to 0.5 increments instead of 0.1, which was
  difficult to land on precisely by touch. Tapping the score number
  itself now lets you type an exact value, including finer decimals than
  the slider snaps to
- Fixed searching Explore by username not returning that user's burgers —
  the search only ever matched restaurant, city, country, and burger
  name, never the poster's username
- Fixed tapping into a friend's burger from their profile showing an
  older version of the detail screen with no comments, likes, or share
  card — a separate, outdated copy of the screen had been in use there
  since before those features existed
- Fixed a burger's displayed like count occasionally showing one higher
  than the real count immediately after liking it, caused by a timing
  race in how the count was optimistically updated
- Fixed new accounts occasionally receiving a burst of duplicate
  "New passport stamp" / badge pop-ups after logging their first burger.
  Two compounding causes: passport stamps weren't checking whether
  they'd already been awarded before showing again, and the one-time
  award check could re-run concurrently with itself under some timing
  conditions, multiplying the duplicates
- Fixed account data (recent burgers, friends, requests, notification
  counts) occasionally persisting briefly from a previous account after
  signing out and into a different one on the same device without fully
  closing the app — most noticeable during testing with multiple
  accounts, but a real correctness issue for anyone switching accounts
